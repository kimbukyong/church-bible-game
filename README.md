...
### 1. 음성 인식 기능 문제
**증상**: 음성 녹음이 안 됨, 마이크 인식 실패

**해결 방법**:
- **브라우저 확인**: Chrome 또는 Edge 브라우저 사용 (Firefox, Safari 미지원)
- **마이크 권한**: 브라우저 주소창 옆 🔒 아이콘 클릭 → 마이크 권한 "허용"으로 설정
- **HTTPS 연결**: 로컬 서버 사용 (`localhost:8000`) 또는 HTTPS 사이트 접속
- **마이크 테스트**: 게임 화면의 "🎤 마이크 테스트" 버튼으로 마이크 상태 확인
- **콘솔 확인**: F12 키 → Console 탭에서 오류 메시지 확인

### 2. Firebase 연결 실패
**증상**: 데이터 저장/불러오기 실패, 연결 오류

**해결 방법**:
- Firebase 설정 값 재확인 (`firebase-config.js`)
- 네트워크 연결 확인
